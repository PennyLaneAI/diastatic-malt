...
# ==============================================================================
"""Thread-local context managers for DiastaticMalt."""
...
def control_status_ctx():
  """Returns the current control context for malt.
  ...
  ```
  def foo(func):
    return tf.__internal__.malt.tf_convert(
       input_fn, ctx=tf.__internal__.malt.control_status_ctx())()
  ```

  Returns:
    The current control context of malt.
  """
  ret = _control_ctx()[-1]
  return ret
...
class ControlStatusCtx(object):
  """A context that tracks whether malt is enabled by the user."""
...
try:
  ...
  ag_logging.warning(
      'DiastaticMalt is not available in this environment: functions lack code'
      ' information. This is typical of some environments like the interactive'
      ' Python shell. See'
      ' https://github.com/pennylaneai/diastatic-malt/blob/main/malt/g3doc/reference/limitations.md#access-to-source-code'
